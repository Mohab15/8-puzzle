...
def bfs(initial_state):
    ...
    while len(frontier) != 0:
        state = frontier.pop(0)    #Poping from a queue
        explore.add(state)
        ...
        if state == "012345678":
            ...
            for child in generate_children(state): #explore neighbours
    ...
    while len(frontier) != 0:
        ...
        if state == "012345678":
            ...
            for child in reversed(generate_children(state)):
                if child not in frontier_state and child not in explore:
                    parent_map[child] = state   # insert into dictionary
                    frontier_state[child] = True
...
#Checks Solvability of inputted configuration According to number of inversions Criteria
...
def generate_children(state):
    #takes a state to get its neighbours and returns them in a list
    state = str(state)
    children = []
    ...
    # Check Move up
    if index > 2:
        children.append(node.swap(state, index, index - 3))    # 0 1 2
                                                               #
    ...
    while len(frontier) > 0:
        ...
        for child in generate_children(state):
            if child not in frontier and child not in explored:
                ...
                if frontier[child] > heuristic(child):
                    frontier[child] = heuristic(child)
...
# Calls the method responsible for each Search method Required
